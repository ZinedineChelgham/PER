...
const PieChartMulti = ({ data, title, x_axis }) => {
    ...
    useEffect(() => {
        ...
        const drawOriginalChart = (chartData) => {
            ...
            arcGroup = g.selectAll('.arc').data(arcs).enter().append('g').attr('class', 'arc');
            ...
            arcGroup
                ...
                .text((d) => {
                    ...
                    let total = Object.values(chartData[d.data.key]).reduce((a, b) => a + b, 0);
        ...
        const drawSubcategoryChart = (subcategories,key) => {
            let subArcs = pie(subcategories.map(([subcategory, value]) => ({ key: subcategory, value })));
            ...
            let subArcGroup = svg
                ...
                .append('g')
                .attr('class', 'arc');
            let total = subcategories.reduce((acc, [subcategory, value]) => acc + value, 0);
            ...
            let legend = svg
                .selectAll('.legend')
                .data(subcategories.map(([subcategory, value]) => ({ subcategory, value })))
                .enter()
                .append('g')
                .attr('class', 'legend')
        ...
        if (selectedOption !== 'Overall') {
            ...
            const arcGroup = g.selectAll('.arc').data(arcs).enter().append('g').attr('class', 'arc');
            const totalResponses = keys.reduce((total, key) => total + plotData[key], 0);
