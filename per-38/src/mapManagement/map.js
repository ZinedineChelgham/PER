...
const drawStatutGraph = (event, d) => {
 ...
 // d3.select('.bar-graph').remove();
  ...
  ids.forEach((id) => {
    const statutEntry = transformedData.find((entry) => entry['Séquentiel'] === id);
...
rectContainer.selectAll('.label')
  ...
  .text(d => d.status == "Maître de Conférences (sans HDR)" ? "MCS"
   : d.status == "Maître de Conférences (avec HDR)" ? "MCA" 
   : d.status == "Professeur des Universités" ? "PU" : "Autre")
  ...
  return (
    <div className="map-container">
      ...
      <DropDown items={['Gender Count', 'Statut Count']} onSelec={handleDropDownChange} />
    </div>
  );
};
